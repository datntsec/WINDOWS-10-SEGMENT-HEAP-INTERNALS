...
Cũng được đề cập là các page range descriptor để mô tả trạng thái của từng trang trong segment. Vì backend block có thể kéo dài nhiều page (một page range), page range descriptor cho trang đầu tiên của backend block được đánh dấu là "first" và do đó, sẽ có các trường bổ sung được set.
```
windbg> dt ntdll!_HEAP_PAGE_RANGE_DESCRIPTOR -r
 ...
 ```
   ...
   - RangeFlags - Trường bit đại diện cho loại backend block và trạng thái của page được đại diện bởi page range descriptor.
   ...
   - Key - Dành cho "first" page range descriptor của free backend blocks. Nó được sử dụng khi một free backend block được chèn vào backend free tree
      - Key - Key với kích thước WORD được sử dụng cho backend free tree. High byte là trường PageCount và low byte là trường EncodedCommitCount(xem thêm bên dưới).
      - EncodedCommitCount - Bitwise NOT của số page được commit của backend block. Số lượng các page được commit mà free backend block có càng lớn, thì EncodedCommitCount sẽ càng nhỏ (Bitwise NOT).
...
Root của backend free tree được lưu trữ trong HeapBase.FreePageRanges và các node trên cây là các "first" page range descriptor của các free backend block. Key được sử dụng để chèn các node trong backend free tree là trường Key.Key của "first" page range descriptor (xem chi tiết về Key.Key ở phần trước).
...
RtlpHpSegPageRangeAllocate() trước tiên đi qua backend free tree để tìm một free backend block có thể phù hợp với phân bổ. Key tìm kiếm được sử dụng để tìm free backend block là một giá trị có kích thước WORD, trong đó high BYTE là số page được yêu cầu và low BYTE là bitwise NOT của số page được yêu cầu. Điều này có nghĩa là một tìm kiếm phù hợp nhất được ưu tiên thực hiện cho block được commit cao nhất, rõ hơn, nếu hai hoặc nhiều free block có kích thước tương đương phù hợp nhất với kích thước cần phân bổ, thì free block được commit cao nhất sẽ được chọn để phân bổ. Nếu bất kỳ free backend blocks nào không thể phù hợp với phân bổ, một segment mới sẽ được tạo.
...
Cuối cùng, trường RangeFlags của page range descriptors của block đó được cập nhật (PAGE_RANGE_FLAGS_ALLOCATED bitis set) để đánh dấu các page của block đó là đã được cấp phát.
...
RtlpHpSegPageRangeShrink() trước tiên sẽ clear bit PAGE_RANGE_FLAGS_ALLOCATED trong trường RangeFlags của tất cả (ngoại trừ “first”) page range descriptors mô tả backend block được giải phóng. Sau đó, nó gọi RtlpHpSegPageRangeCoalesce() để hợp nhất backend block được giải phóng với các free backend block lân cận (trước và sau) và clear bit PAGE_RANGE_FLAGS_ALLOCATED trong trường RangeFlags của “first” page range descriptor của block được giải phóng.
...
```
   - Sizes - Cấu trúc con có kích thước QWORD được mã hóa, nó chứa thông tin quan trọng về kích thước và trạng thái
      - MemoryCost - Được dùng cho free VS blocks. Một giá trị được tính dựa trên độ lớn của phần được commit của block. Phần block được commit càng lớn thì có memory cost càng thấp. Điều này có nghĩa là nếu một block có memory cost thấp được chọn để cấp phát, thì lượng bộ nhớ cần được commit sẽ nhỏ hơn. 
...
Trước tiên, RtlpHpVsContextAllocate() duyệt VS free tree để tìm một free VS block có thể phù hợp với phân bổ. Key tìm kiếm được sử dụng để tìm free VS block là một giá trị có kích thước DWORD trong đó high WORD là số lượng block 16 byte có thể chứa AllocSize (kích thước phân bổ) cộng thêm một (thêm block header) và low WORD là 0 (đối với MemoryCost). Điều này có nghĩa là một tìm kiếm phù hợp nhất được thực hiện với free VS block với memory cost thấp nhất (hầu hết các phần của block được commit) được ưu tiên, nói cách khác, nếu hai hoặc nhiều free block có kích thước tương đương phù hợp nhất với phân bổ, free block được commit nhiều nhất sẽ được chọn để phân bổ. Nếu không có bất kỳ free VS block nào phù hợp với phân bổ, một VS segment mới sẽ được tạo.

Vì kích thước của free VS block đã chọn có thể lớn hơn kích thước block có thể chứa AllocSize, các free VS block lớn sẽ được tách ra ngoại trừ khi kích thước block của block còn lại nhỏ hơn 0x20 byte (kích thước của free VS block header).

![](pic/pic19.PNG)

Việc tách free VS block được thực hiện bởi hàm RtlpHpVsChunkSplit(). RtlpHpVsChunkSplit() cũng là hàm loại bỏ free VS block khỏi VS free tree và cũng chèn free block còn lại sau khi tách vào VS free tree nếu có thể tách block.
...
Trong đó UserAddress là địa chỉ của VS block được giải phóng và LfhBlockSize sẽ trở thành block size của VS block được giải phóng trừ đi 0x10 (kích thước busy VS block header). LfhBlockSize sẽ được dùng bởi việc gọi hàm RtlpHpVsContextFree() sử dụng trong việc cập nhật usage counter của LFH bucket tương ứng với LfhBlockSize.
...
Cuối cùng, free block được liên kết được chèn vào VS free tree để dùng cho việc phân bổ.
...
Các LFH bucket chỉ được kích hoạt (enabled) nếu kích thước phân bổ tương ứng của nó được phát hiện dùng nhiều lần. LFH bucket activation và usage counter sẽ được thảo luận kỹ hơn ở phần sau.
...
LFH component dựa vào backend để tạo các LFH subsegment nơi các LFH block được cấp phát từ đó. Một LFH subsegment là một loại đặc biệt của backend block trong đó trường RangeFlags của “first” page range descriptor tương ứng có PAGE_RANGE_FLAGS_LFH_SUBSEGMENT (0x01) bit được set.
...
```
   - BackendCtx - trỏ đến cấu trúc _SEGMENT_HEAP (HeapBase).
   - Callbacks – Các callback được mã hóa (xem thêm ở phần [3.5](https://github.com/datntsec/WINDOWS-10-SEGMENT-HEAP-INTERNALS/blob/master/README.md#35-function-pointer-encoding)) để quản lý các phần mở rộng của LFH subsegments và LFH context.
...
```
   - Invalid - Điểm đánh dấu để xác định xem con trỏ này có phải là con trỏ _HEAP_LFH_BUCKET không hợp lệ hay không (lowest bit set), từ đó xác định cấu trúc này là một bucket usage counter.
...
Bởi vì ban đầu chỉ có một affinity slot được tạo, điều đó có nghĩa là tất cả các bộ xử lý sẽ sử dụng cùng một tập hợp các LFH subsement và do đó, có thể xảy ra contention. Nếu quá nhiều contention xảy ra, một affinity slot mới sẽ được tạo và bộ xử lý của luồng yêu cầu được chỉ định lại cho affinity slot mới thông qua trường ProcAffinityMapping trong bucket.
...
```
   ...
   - Owner - Con trỏ trỏ đến affinity slot sở hữu LFH subsegment.
   ...
   - BlockOffsets - Cấu trúc con có kích thước DWORD được mã hóa (xem thêm ở phần [3.7](https://github.com/datntsec/WINDOWS-10-SEGMENT-HEAP-INTERNALS/blob/master/README.md#37-lfh-subsegment-blockoffsets-encoding)) chứa kích thước của mỗi LFH block và offset của LFH block đầu tiên trong LFH subsegment.
   ...
   - CommitStateOffset - Offset của mảng commit state trong LFH subsegment. Một LFH subsegment được chia thành nhiều “commit portions”; commit state là một mảng các giá trị có kích thước WORD đại diện cho commit state của mỗi “commit portions”.
...
Mỗi LFH block được biểu diễn bằng hai bit trong block bitmap. Bit 0 đại diện cho BUSY bit và bit 1 đại diện cho UNUSED BYTES bit. Nếu UNUSED BYTES bit được set, điều đó có nghĩa là có sự khác biệt giữa UserSize và LFH block size, và hai byte cuối cùng của LFH block được coi là giá trị 16 bit endian low để thể hiện sự khác biệt, nghĩa là nếu unused bytes là 0, high bit của giá trị 16 bit này được set và phần còn lại của các bit này không được sử dụng, ngược lại, high bit là clear, 14 bit thấp được sử dụng để lưu trữ giá trị unused byte.

Block bitmap cũng được chia thành các chunk có kích thước QWORD (64 bit), được gọi là BitmapBits trong bài viết này, với mỗi BitmapBit đại diện cho 32 LFH block.
...
Trong mọi yêu cầu phân bổ có kích thước phân bổ <= 16.368 (0x3FF0) byte, RtlpHpLfhContextAllocate() trước tiên được gọi để kiểm tra xem bucket tương ứng với kích thước phân bổ có được kích hoạt hay không. Nếu bucket được kích hoạt, việc phân bổ sẽ được thực hiện bởi LFH.
...
Kích hoạt bucket xảy ra nếu có 17 cấp phát đang hoạt động (chưa free) cho kích thước phân bổ của bucket. Cấp phát đang hoạt động thứ 17 sẽ kích hoạt bucket, nó và các cấp phát tiếp theo sau đó sẽ do LFH thực hiện.

Kích hoạt bucket cũng xảy ra nếu có 2.040 yêu cầu cấp phát cho kích thước phân bổ của bucket, bất kể các block từ việc cấp phát trước đó đã được giải phóng hay chưa. Việc cấp phát block thứ 2.040 sẽ kích hoạt bucket, nó và các cấp phát tiếp theo sau đó sẽ được thực hiện bởi LFH.
...
Dưới đây là hình minh họa trong đó 8 LFH block được phân bổ tuần tự từ một new LFH subsegment, hãy chú ý vị trí ngẫu nhiên của mỗi phân bổ LFH:
...
Tiếp theo, các bit BUSY và UNUSED BYTES tương ứng của LFH block trong block bitmap được atomically clear. Sau đó, trường FreeCount của LFH subsegment được tăng atomically làm cho LFH block sẵn sàng để phân bổ.
...
Việc phân bổ large block rất đơn giản vì không có free tree/list nào để tham khảo. Đầu tiên, việc phân bổ metadata của block từ metadata heap được thực hiện. Tiếp theo, thông qua NtAllocateVirtualMemory(), một virtual memory có kích thước bằng kích thước phân bổ cộng với 0x1000 byte cho guard page sẽ được reserve. Sau đó, một kích thước bằng với kích thước cấp phát được commit từ bộ nhớ được reserve ban đầu, để guard page cuối cùng vẫn ở reserved state.

Sau khi phân bổ block, các trường metadata của block được set và large allocation bitmap (RtlpHpLargeAllocationBitmap) được cập nhật để đánh dấu địa chỉ của block (thực ra là UserAddress >> 16) là một large block allocation.
...
Tiếp theo, large allocation bitmap được cập nhật để bỏ đánh dấu địa chỉ của block là một large block allocation. Sau đó, virtual memory của block được giải phóng và metadata của block được giải phóng.

### 2.6. BLOCK PADDING
Trong các ứng dụng không được chọn tham gia theo mặc định để sử dụng Segment Heap (tức là: không phải ứng dụng Windows và không phải là file thực thi hệ thống như đã thảo luận trong phần [2.1](https://github.com/datntsec/WINDOWS-10-SEGMENT-HEAP-INTERNALS/blob/master/README.md#21-overview)), một vùng padding 16 (0x10) byte bổ sung được thêm vào block. Phần padding làm tăng tổng kích thước block cần thiết để phân bổ và thay đổi bố cục của backend block, VS block và LFH block.
...
Việc ngẫu nhiên hóa địa chỉ heap được thực hiện bởi hàm RtlpHpSegHeapAllocate(), được sử dụng để tạo heap. Nó được thực hiện bằng cách reserving virtual memory có kích thước bằng lấy với kích thước của heap cộng với kích thước được tạo ngẫu nhiên (kích thước ngẫu nhiên là bội số của 64KB). Sau khi reserving virtual memory, phần đầu của reserved virtual memory có kích thước bằng với kích thước ngẫu nhiên được tạo ban đầu sẽ được free. Sau đó, HeapBase được trỏ đến sau phần đã được free của reserved virtual memory ban đầu.
...
Để bảo vệ các trường LFH subsegment header quan trọng khỏi sửa đổi nhỏ, trường block size và trường first block offset trong LFH subsegment header được mã hóa bằng cách sử dụng LFH key và LFH subsegment address.

![](pic/pic38.PNG)

### 3.8. LFH ALLOCATION RANDOMIZATION
Để làm cho việc khai thác buffer overflows dựa trên LFH và use-after-frees (UAF) trở nên unreliable, thành phần LFH sẽ chọn ngẫu nhiên một free LFH block để sử dụng trong một yêu cầu cấp phát. Việc phân bổ ngẫu nhiên làm cho việc đặt target LFH block liền kề với LFH block có thể bị overflow là unreliable và nó cũng khiến việc sử dụng lại LFH block được giải phóng gần đây (UAF) cũng trở thành unreliable. Thuật toán phân bổ ngẫu nhiên đã được thảo luận trong phần [2.4](https://github.com/datntsec/WINDOWS-10-SEGMENT-HEAP-INTERNALS/blob/master/README.md#24-low-fragmentation-heap) “LFH Allocation”.
...
Các cơ chế bảo mật được áp dụng trong Segment Heap chủ yếu là sự chuyển giao các cơ chế bảo mật từ NT Heap, trong đó đáng chú ý là các guard page và ngẫu nhiên phân bổ LFH mới xuất hiện khi Windows 8 được phát hành. Dựa trên điều này, và cách các trường quan trọng của cấu trúc dữ liệu mới được bảo vệ, Segment Heap có thể so sánh với NT Heap về cơ chế bảo mật được áp dụng là tốt hơn. Tuy nhiên, vẫn chưa thấy cấu trúc dữ liệu Segment Heap mới sẽ hoạt động như thế nào khi nghiên cứu tấn công metadata của Segment Heap dần trở nên phổ biến.

Liên quan đến thao tác sắp xếp bố cục heap, thuật toán tìm kiếm phù hợp nhất và cơ chế tách free block của backend và VS component cho thao tác bố trí heap được sử dụng nhiều hơn so với LFH component sử dụng ngẫu nhiên hóa phân bổ.
...
Nếu kẻ tấn công có thể cấy ghép một địa chỉ target ngay sau phần cuối của ngăn xếp toán hạng PostScript, kẻ tấn công sẽ có thể thực hiện ghi bộ nhớ vào địa chỉ đích thông qua hoạt động PostScript để push một giá trị trong ngăn xếp toán hạng PostScript.
...
Giải phóng các block liên quan đến việc loại bỏ các tham chiếu đến đối tượng ArrayBuffer và sau đó kích hoạt một garbage collection. Lưu ý rằng Chakra’s CollectGarbage() vẫn có thể gọi được nhưng chức năng của nó đã bị tắt trong Edge, do đó, cần một cơ chế khác để kích hoạt garbage collection.
...
Do đó, để thực hiện garbage collection, một ArrayBuffer có kích thước 192MB được tạo, sau đó tạo một delay để chờ quá trình garbage collection diễn ra đồng thời kết thúc và sau đó, thực thi một mã JavaScript tiếp theo:
``` c 
...
Để giải quyết vấn đề này, một thuộc tính của VS chunk splitting có thể được tận dụng. Cụ thể, như đã đề cập trước đây trong phần “VS Allocation” ở [2.3](https://github.com/datntsec/WINDOWS-10-SEGMENT-HEAP-INTERNALS/blob/master/README.md#23-variable-size-allocation), các large free block được tách ra ngoại trừ khi block size của block còn lại sẽ nhỏ hơn 0x20 byte.

Do đó, nếu một controlled buffer 0x340 byte (tổng block size bao gồm cả header là 0x350) được sử dụng và ngăn xếp toán hạng PostScript 0x328 byte (tổng block size bao gồm cả header là 0x340) sẽ được phân bổ trong free VS block của controlled buffer được giải phóng, thì kích thước của block còn lại sau khi tách sẽ chỉ còn 0x10 byte, do đó ngăn chặn việc chia free VS block 0x350 byte. Và nếu đúng như vậy, giá trị địa chỉ target được lưu trữ tại unused bytes ở offset 0x33E của VS block sẽ không bị sửa đổi:

![](pic/pic45.PNG)

### 4.5. PREVENTING FREE BLOCKS COALESCING
Để ngăn free VS block của controlled buffer được giải phóng hợp nhất với các free VS blocks lân cận, 15 (thay vì một) controlled buffers được tạo tuần tự, sau đó, ta sẽ free xen kẽ để giữ lại tám buffers (busy) và chỉ free bảy buffers.
...
Việc phân bổ thực tế không phải lúc nào cũng khớp chính xác với hình minh họa ở trên, chẳng hạn như khi một số controlled buffer được cấp phát từ một  VS subsegment khác. Tuy nhiên, nhiều controlled buffer được free và busy làm tăng khả năng ít nhất một hoặc nhiều free VS blocks của controlled buffers được giải phóng sẽ không được hợp nhất với nhau.
...
Bây giờ các giải pháp cho các vấn đề trên đã được giải quyết, kế hoạch ban đầu để cấy ghép địa chỉ target được điều chỉnh như sau:
   ...
   3. HTML/JavaScript: Theo cách xen kẽ, free bảy controlled buffers và để lại tám controlled buffers (busy).
...
Hai yếu tố chính cho phép thao tác chính xác bố cục heap trong case study này là khả năng viết tập lệnh được cung cấp bởi công cụ Chakra JavaScript và một heap chung được sử dụng bởi cả ArrayBuffer của Chakra’s và trình thông dịch PostScript của WinRT PDF. Nếu không có hai yếu tố này, thao tác bố cục chính xác của MSVCRT heap bằng cách sử dụng internal allocation và freeing của WinRT PDF của các đối tượng có thể sẽ khó khăn hơn.
...
Cuối cùng, tôi hy vọng rằng bài viết này sẽ giúp bạn hiểu rõ Segment Heap của Windows 10.
